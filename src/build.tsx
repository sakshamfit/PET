/**
 * Build identity, staleness detection and the "new version is live" banner
 * for the school-portal web build (the static bundle Vercel serves).
 *
 * The browser keeps whatever it last downloaded, and until now nothing in the
 * app could say which build that was — that is how you end up staring at an
 * old Vercel deployment with no way to tell. So, mirroring the PET app:
 *
 *   1. every build embeds its own id (vite.config.ts → __LEGACY_BUILD__);
 *   2. the build writes `build-info.json` next to the bundle;
 *   3. this module shows that id in the UI and polls the server for it, so a
 *      client that is behind says so, out loud, with a Reload button.
 *
 * The watcher only runs on http(s) (Vercel / dev server). The Electron desktop
 * and Android shells load the bundle from disk, where there is nothing to poll.
 */

import { useCallback, useEffect, useState } from 'react';

export type LegacyBuildInfo = {
  schema?: number;
  app?: string;
  version: string;
  buildId: string;
  commit: string | null;
  sourceHash: string | null;
  sourceFiles?: number;
  builtAt: string;
};

declare const __LEGACY_BUILD__: LegacyBuildInfo | undefined;

const FALLBACK: LegacyBuildInfo = {
  version: '0.0.0',
  buildId: 'dev',
  commit: null,
  sourceHash: null,
  builtAt: new Date(0).toISOString(),
};

/** The build this bundle was compiled from (injected by Vite `define`). */
export const BUILD: LegacyBuildInfo =
  typeof __LEGACY_BUILD__ === 'object' && __LEGACY_BUILD__ ? { ...FALLBACK, ...__LEGACY_BUILD__ } : FALLBACK;

/**
 * URL of the server's build descriptor.
 *
 * Derived from this bundle's own URL so it stays correct whatever host or
 * path the app is served from: the bundle lives at <base>/assets/index-<hash>.js,
 * so the descriptor lives at <base>/build-info.json.
 */
function buildInfoUrl(): string {
  const ASSET_MARKER = '/assets/';
  try {
    const self = new URL(import.meta.url);
    const at = self.pathname.indexOf(ASSET_MARKER);
    if (at > -1) {
      return `${self.origin}${self.pathname.slice(0, at + 1)}build-info.json`;
    }
  } catch {
    /* not running from a URL (unit tests / SSR) */
  }
  return '/build-info.json';
}

const POLL_MS = 60_000;

export type BuildWatch = {
  /** build id currently served by the server (null until known / unreachable) */
  serverBuildId: string | null;
  /** true when the server serves a different build than this page is running */
  updateAvailable: boolean;
  /** this page's build */
  clientBuildId: string;
  /** force a refresh — reloads the shell from the network */
  applyUpdate: () => void;
};

/**
 * Polls the server for its current build id. Safe offline (silently ignores
 * failures) and safe in dev (the dev server has no build-info.json) and in the
 * desktop/Android shells (no http origin — stays quiet).
 */
export function useBuildWatcher(enabled = true): BuildWatch {
  const [serverBuildId, setServerBuildId] = useState<string | null>(null);
  const [updateAvailable, setUpdateAvailable] = useState(false);

  useEffect(() => {
    if (!enabled) return;
    if (typeof window === 'undefined') return;
    if (!window.location.protocol.startsWith('http')) return;
    let cancelled = false;
    const url = buildInfoUrl();

    async function check() {
      if (typeof navigator !== 'undefined' && navigator.onLine === false) return;
      try {
        const res = await fetch(`${url}?t=${Date.now()}`, { cache: 'no-store' });
        if (!res.ok) return;
        const type = res.headers.get('content-type') || '';
        if (!type.includes('json')) return; // dev server SPA fallback — not a build
        const info = (await res.json()) as LegacyBuildInfo;
        if (cancelled || !info?.buildId) return;
        setServerBuildId(info.buildId);
        setUpdateAvailable(info.buildId !== BUILD.buildId && BUILD.buildId !== 'dev');
      } catch {
        /* offline or dev — never nag the user about it */
      }
    }

    void check();
    const timer = window.setInterval(check, POLL_MS);
    const onWake = () => {
      if (document.visibilityState === 'visible') void check();
    };
    document.addEventListener('visibilitychange', onWake);
    window.addEventListener('online', check);
    window.addEventListener('focus', onWake);
    return () => {
      cancelled = true;
      window.clearInterval(timer);
      document.removeEventListener('visibilitychange', onWake);
      window.removeEventListener('online', check);
      window.removeEventListener('focus', onWake);
    };
  }, [enabled]);

  const applyUpdate = useCallback(() => {
    window.location.reload();
  }, []);

  return { serverBuildId, updateAvailable, clientBuildId: BUILD.buildId, applyUpdate };
}

/** Small, unobtrusive build stamp — the answer to "which build am I looking at?" */
export function BuildStamp({ className = '' }: { className?: string }) {
  const built = BUILD.builtAt && BUILD.builtAt !== FALLBACK.builtAt ? new Date(BUILD.builtAt) : null;
  return (
    <p
      className={`text-[10px] leading-tight text-[#86868b] ${className}`}
      title={`Build ${BUILD.buildId}\nBuilt ${built ? built.toLocaleString() : 'unknown'}${BUILD.commit ? `\nCommit ${BUILD.commit}` : ''}`}
    >
      v{BUILD.version} · build {BUILD.buildId}
    </p>
  );
}

/**
 * Persistent banner shown when the server has a newer build than this page.
 * Nothing auto-reloads underneath the user — they may be mid-form.
 */
export function UpdateBanner({ watch }: { watch: BuildWatch }) {
  if (!watch.updateAvailable) return null;
  return (
    <div className="sticky top-0 z-50 flex items-center justify-between gap-3 bg-[#0066cc] px-4 py-2 text-xs font-semibold text-white shadow">
      <span>A newer version of this app is live — reload to get it.</span>
      <button
        type="button"
        className="rounded-full bg-white/20 px-3 py-1 font-semibold text-white ring-1 ring-white/40 hover:bg-white/30 transition"
        onClick={watch.applyUpdate}
      >
        Update now
      </button>
    </div>
  );
}
